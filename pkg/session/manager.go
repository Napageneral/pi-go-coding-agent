...
type Manager struct {
	...
	byID        map[string]int
	leafID      string
	inMemory    bool // when true, skip all disk I/O
}
...
// NewInMemory creates a session manager that never touches disk.
// Use AppendMessage to seed conversation history from an external source
// (e.g. a broker ledger), then pass the manager to a Runtime.
func NewInMemory(cwd string) *Manager {
	m := &Manager{
		inMemory: true,
		byID:     map[string]int{},
		header: Header{
...
// InMemory reports whether this manager skips disk persistence.
